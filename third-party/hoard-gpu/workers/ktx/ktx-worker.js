...
const KTX_DECODER = new Promise((resolve) => {
...
class KtxDecoderService extends WorkerPoolService {
  async init() {
    this.ktx = await KTX_DECODER;
  }

  async onDispatch(args) {
    const ktx = this.ktx;
    const ktxTexture = new ktx.ktxTexture(new Uint8Array(args.arrayBuffer));
    const supportedFormats = args.supportedFormats;

    // Uncompressed is always supported, but never ideal.
    supportedFormats.push('rgba8unorm');
    ...
    const textureData = {
      ...
      size: {
        ...
        depthOrArrayLayers: ktxTexture.numLayers * ktxTexture.numFaces,
      },
    ...
    for (let level = 0; level < ktxTexture.numLevels; ++level) {
      let levelSize = {
        width: Math.max(1, ktxTexture.baseWidth >> level),
        height: Math.max(1, ktxTexture.baseHeight >> level),
      };
      for (let layer = 0; layer < ktxTexture.numLayers; ++layer) {
        for (let face = 0; face < ktxTexture.numFaces; ++face) {
          imageData[sliceIndex] = ktxTexture.getImageData(level, layer, face);
          textureData.bufferViews[sliceIndex] = {
            levelSize,
            level,
            layer: (layer * ktxTexture.numFaces) + face,
            byteOffset: totalSize,
            byteLength: imageData[sliceIndex].byteLength,
          };
          totalSize += imageData[sliceIndex].byteLength;
          sliceIndex++;
        }
      }
    }

    // TODO: Copy layer/face data at the appropriate stride to do a single copy
    // per level in WebGPU.
    const textureBuffer = new Uint8Array(totalSize);
    textureData.arrayBuffer = textureBuffer.buffer;
    for (const sliceIndex in imageData) {
      textureBuffer.set(imageData[sliceIndex], textureData.bufferViews[sliceIndex].byteOffset);
    }

    ktxTexture.delete();

    return this.transfer(textureData, [textureData.arrayBuffer]);
  }
}

// Initialize the service
new KtxDecoderService();
